...
def main_gen_soc_lib_include(file):
    ...
    # We don't need to include memory.h in Zircon
...
def main_block_1(file, unroll, debug):
    ...
    file.write("\n")
    file.write("    HAL_Cgra_Glc_WriteReg(GLC_CGRA_STALL_R, 0xFFFFFFF);\n")
    file.write("\n")
    if(debug):
...
def main_block_2(file, mapping_dict, op_list, unroll, glb_tile_offset, glb_bank_offset, debug):
    ...
    # Fluch crossbar is included in reg_write.h, which is newly included in Zircon
    # if(unroll != "0"):
    ...
    file.write("    HAL_Cgra_Glc_WriteReg(GLC_STRM_F2G_IER_R, 0xfffffff);\n")
    file.write("    HAL_Cgra_Glc_WriteReg(GLC_STRM_G2F_IER_R, 0xfffffff);\n")
    file.write("\n")
    if(debug):
        file.write("    trace_printf(\"\\n** Run code-gen  **\\n\");\n")
    else:
