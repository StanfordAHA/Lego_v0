...
def unrolling(inputs, outputs, input_order, output_order, f, app_name, unroll):
    ...
    if(unroll):
        ...
        for idx, input_name in enumerate(inputs):
            ...
            f.write(f"  HAL_Cgra_Glb_WriteReg(0x100*8 + 0x100 * ({input_name_str}_unroll_array[0]) + GLB_LD_DMA_HEADER_0_START_ADDR_R, 0x40000*8 + 0x40000 * ({input_name_str}_unroll_array[0]) + {input_name_str}_extents_sum_unroll);\n")
