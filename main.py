...
def cp_closing_decleration(main_file, cg_source_id, cg_source_map, op_list, mode, dest_id, unroll, mapping_dict=None):
    ...
    if(mode == 'onyx'):
        ...
        if(unroll): 
            main_file.write("        " + "codegen_check_gold_unroll_ifdef_close(output_gold_file);\n")
...
if __name__ == "__main__":
    ...
    if mode == "onyx":
        ...
        main_block_1(main_file, debug)
        main_block_2(main_file, mapping_dict, op_list, debug)
        main_block_3(main_file, mapping_dict, dest_read, debug, gcheck)    
