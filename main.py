...
if __name__ == "__main__":
    ...
    parser.add_argument("--dense_iter", action="store_true")
    ...
    if(args.dense_iter):
        ...
        tensor = os.path.abspath(args.tensor)
        ...
        if args.positive_only: 
            ...
            positive_only = ""
        mode = "rtl"
        comal_batch_size = args.comal_batch_size
        gold_check = args.gold_check
        if args.workspace:
            workspace = "--workspace" 
        else:
            workspace = ""
        ...
        # Call the main.py in the dense_patch/ with these arguments 
        os.chdir("./dense_patch")
        os.system(f"python3 main.py -t {tensor} -p {program} {positive_only} -m {mode} -b {comal_batch_size} -g {gold_check} {workspace} -o {output_dir}")

        # Change the working directory back to the original directory
        os.chdir("..")
        
        # Check if main.cpp exists in the dense_patch directory
        if os.path.exists("./dense_patch/main.cpp"):
            if os.path.exists("./main.cpp"):
                os.remove("./main.cpp")
            shutil.copy2("./dense_patch/main.cpp", "./")
            for item in os.listdir("./dense_patch/lego_scratch"):
                s = os.path.join("./dense_patch/lego_scratch", item)
                d = os.path.join("./lego_scratch", item)
                if os.path.isdir(s):
                    shutil.move(s, d)
                else:
                    shutil.copy2(s, d)
        else:
            print("main.cpp not found in dense_patch directory.")

        if args.gold_check == "s": 
            if os.path.exists("./dense_patch/gold.cpp"):
                if os.path.exists("./gold.cpp"):
                    os.remove("./gold.cpp")
                shutil.copy2("./dense_patch/gold.cpp", "./")
            else:
                print("gold.cpp not found in dense_patch directory.")
    else:
