...
def process(tensor_type, input_path, output_dir_path, tensor_size, schedule_dict, format, gen_tensor, nnz, gold_check, dtype, app_name):
    ...
    if tensor_type == "gen":
        ...
        if(dtype == "float"):
            tensor = sparse.random(size, nnz=nnz)
    elif tensor_type == "ex":
