...
def cp_op_stmt(op_list, sub_point, id_dict, id_dict_true, level, curr_id, mode, split_dict, cg_source_id, dest, cg_source_map, workspace, unroll, lut_tensor, dtype, tensor_format_dict):
        stmt = ""
...
def lower(stmt, id_dict, id_dict_true, op_list, schedule, level, target, split_dict, dest, mode, next_id_dict, next_id_map, scalar, workspace, process_csf, unroll, lut_tensor, dtype, tensor_format_dict):
    curr_id = schedule[0]
    stmt_list = []
        ...
        for sub_point in sub_points:
            ...
            if(len(schedule) == 1):
                if(target == "ap"):
                    ...
                    stmt_list.append(cp_op_stmt(op_list, sub_point, id_dict, id_dict_true, level, curr_id, mode, split_dict, next_id_dict, dest, next_id_map, workspace, unroll, lut_tensor, dtype, tensor_format_dict))
                elif(target == "cg"):
                    stmt_list.append(cg_op_stmt(op_list, sub_point, id_dict, id_dict_true, level, curr_id, stmt, dest, split_dict, scalar, dtype))
            else:     
                stmt_list.extend(lower(stmt, sub_point_id_dict, id_dict_true, op_list, sub_point_schedule, level + 2, target, split_dict, dest, mode, next_id_dict, next_id_map, scalar, workspace, process_csf, unroll, lut_tensor, dtype, tensor_format_dict))
