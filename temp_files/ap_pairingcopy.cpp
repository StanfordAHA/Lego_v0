...
void print_subtile(int **subtile_B_ptrs){

    int *pos1 = subtile_B_ptrs[0];
    int *crd1 = subtile_B_ptrs[1];
    int *pos2 = subtile_B_ptrs[2];
    int *crd2 = subtile_B_ptrs[3];
...
int ap_operate(int **B_ptrs, int **C_ptrs){ 
    ...
    int *store_B = (int*) malloc(store_size * sizeof(int));
    int *store_C = (int*) malloc(store_size * sizeof(int));

    memset(store_B, -1, store_size * sizeof(int));
    memset(store_C, -1, store_size * sizeof(int));
            ...
            if(iB0 == i) {
                ...
                while(kB < pB2_end && kC < pC1_end) {
                        ...
                        if(kB0 == k && kC0 == k) {    
                            ...
                            if(store_B[i * 8 + k] == -1){
                                store_B[i * 8 + k] = 1;
                                subtile_slice(B_ptrs, kB, B_mode_0, B_mode_1, B_mode_vals, B_extents_mode_0, B_extents_mode_1, B_extents_mode_vals);
                                B_mode0_start[i * 8 + k] = B_extents_mode_0[0];
                                B_mode1_start[i * 8 + k] = B_extents_mode_1[0];
                                B_mode_vals_start[i * 8 + k] = B_extents_mode_vals[0];
                                B_mode0_end[i * 8 + k] = B_extents_mode_0[1];
                                B_mode1_end[i * 8 + k] = B_extents_mode_1[1];
                                B_mode_vals_end[i * 8 + k] = B_extents_mode_vals[1];                                
                            }
                            int jC = C2_pos[kC];
                            ...
                            while(jC < pC2_end) {
                                    ...
                                    if(jC0 == j) {
                                        ...
                                        printf("Si = %d, Sk = %d, Sj = %d\n", i, k, j);

                                        print_subtile(stile_B_ptrs); 
                                        print_subtile(stile_C_ptrs);
                                        ...
                                        if(store_C[k * 8 + j] == -1){
                                            store_C[k * 8 + j] = 1;
                                            subtile_slice(C_ptrs, jC, C_mode_0, C_mode_1, C_mode_vals, C_extents_mode_0, C_extents_mode_1, C_extents_mode_vals);
                                            C_mode0_start[k * 8 + j] = C_extents_mode_0[0];
                                            C_mode1_start[k * 8 + j] = C_extents_mode_1[0];
                                            C_mode_vals_start[k * 8 + j] = C_extents_mode_vals[0];
                                            C_mode0_end[k * 8 + j] = C_extents_mode_0[1];
                                            C_mode1_end[k * 8 + j] = C_extents_mode_1[1];
                                            C_mode_vals_end[k * 8 + j] = C_extents_mode_vals[1];      
                                        }

                                        B_extents_mode_0_run.push_back(B_mode0_start[i * 8 + k]);
                                        B_extents_mode_0_run.push_back(B_mode0_end[i * 8 + k]);

                                        B_extents_mode_1_run.push_back(B_mode1_start[i * 8 + k]);
                                        B_extents_mode_1_run.push_back(B_mode1_end[i * 8 + k]);

                                        B_extents_mode_vals_run.push_back(B_mode_vals_start[i * 8 + k]);
                                        B_extents_mode_vals_run.push_back(B_mode_vals_end[i * 8 + k]);

                                        C_extents_mode_0_run.push_back(C_mode0_start[k * 8 + j]);
                                        C_extents_mode_0_run.push_back(C_mode0_end[k * 8 + j]);

                                        C_extents_mode_1_run.push_back(C_mode1_start[k * 8 + j]);
                                        C_extents_mode_1_run.push_back(C_mode1_end[k * 8 + j]);

                                        C_extents_mode_vals_run.push_back(C_mode_vals_start[k * 8 + j]);
                                        C_extents_mode_vals_run.push_back(C_mode_vals_end[k * 8 + j]); 
...
int main(){
    ...
    ifstream input_file10("tensors/lego_test_tiles/tiled_CSF_c_jk/tile_tiled_csf_pos_0_0.txt");
    load_vector(C1_pos, input_file10);

    ifstream input_file11("tensors/lego_test_tiles/tiled_CSF_c_jk/tile_tiled_csf_crd_0_0.txt");
    load_vector(C1_crd, input_file11);

    ifstream input_file12("tensors/lego_test_tiles/tiled_CSF_c_jk/tile_tiled_csf_pos_0_1.txt");
    load_vector(C2_pos, input_file12);

    ifstream input_file13("tensors/lego_test_tiles/tiled_CSF_c_jk/tile_tiled_csf_crd_0_1.txt");
    load_vector(C2_crd, input_file13);

    ifstream input_file14("tensors/lego_test_tiles/tiled_CSF_c_jk/tile_tiled_csf_pos_1_0.txt");
    load_vector(C3_pos, input_file14);

    ifstream input_file15("tensors/lego_test_tiles/tiled_CSF_c_jk/tile_tiled_csf_crd_1_0.txt");
    load_vector(C3_crd, input_file15);

    ifstream input_file16("tensors/lego_test_tiles/tiled_CSF_c_jk/tile_tiled_csf_pos_1_1.txt");
    load_vector(C4_pos, input_file16);

    ifstream input_file17("tensors/lego_test_tiles/tiled_CSF_c_jk/tile_tiled_csf_crd_1_1.txt");
    load_vector(C4_crd, input_file17);

    ifstream input_file18("tensors/lego_test_tiles/tiled_CSF_c_jk/tile_tiled_csf_data.txt");
